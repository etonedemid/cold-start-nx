...
void CutsceneEditor::shutdown() {
    // fonts are shared; don't close them here
}
...
void CutsceneEditor::drawBevel(int x, int y, int w, int h, bool raised) {
    SDL_Color hi  = raised ? SDL_Color{220,220,220,255} : SDL_Color{80,80,80,255};
    SDL_Color lo  = raised ? SDL_Color{80,80,80,255}    : SDL_Color{220,220,220,255};
...
void CutsceneEditor::renderCutsceneList(int x, int y, int w, int h) {
    const SDL_Color bgDark   = {28,  28,  28,  255};
    const SDL_Color bgSel    = {60,  90,  150, 255};
    const SDL_Color colText  = {200, 200, 200, 255};
    const SDL_Color colHead  = {160, 160, 160, 255};
    ...
    // + button at bottom
    int btnY = y + h - 20;
    fillRect(x+2, btnY, 40, 16, {50,50,50,255});
    drawBevel(x+2, btnY, 40, 16, true);
    drawText("+ Add", x+5, btnY+2, colText, 11);
    if (selectedCutscene_ >= 0) {
        fillRect(x+46, btnY, 40, 16, {50,50,50,255});
        drawBevel(x+46, btnY, 40, 16, true);
        drawText("Del", x+52, btnY+2, colText, 11);
    }
}

void CutsceneEditor::renderActorList(int x, int y, int w, int h) {
    const SDL_Color bgDark   = {32,  32,  32,  255};
    const SDL_Color bgSel    = {70,  100, 40,  255};
    const SDL_Color colText  = {200, 200, 200, 255};
    const SDL_Color colHead  = {160, 160, 160, 255};
    const SDL_Color colType  = {120, 180, 255, 255};
    ...
    // + button at bottom
    int btnY = y + h - 20;
    fillRect(x+2, btnY, 60, 16, {50,50,50,255});
    drawBevel(x+2, btnY, 60, 16, true);
    drawText("+ Actor", x+5, btnY+2, colText, 11);
    if (selectedActor_ >= 0) {
        fillRect(x+66, btnY, 40, 16, {50,50,50,255});
        drawBevel(x+66, btnY, 40, 16, true);
        drawText("Del", x+72, btnY+2, colText, 11);
    }

    // Actor type submenu
    if (showActorMenu_) {
        int mx = x+2, my = btnY - 72;
        fillRect(mx, my, 90, 70, {50,50,50,255});
...
void CutsceneEditor::renderTimeline(int x, int y, int w, int h) {
    const SDL_Color bgMain  = {20, 20, 20, 255};
    const SDL_Color bgRow   = {28, 28, 28, 255};
    const SDL_Color bgRowAlt= {24, 24, 24, 255};
    const SDL_Color colGrid = {45, 45, 45, 255};
    const SDL_Color colText = {160,160,160,255};
    const SDL_Color colScrub= {255,200,0,  255};
    const SDL_Color colSel  = {255,255,255,255};
    ...
    // Play/pause
    SDL_Color playCol = playing_ ? SDL_Color{100,220,100,255} : SDL_Color{180,180,180,255};
    fillRect(x+4, toolY+3, 16, 16, {50,50,50,255});
    drawBevel(x+4, toolY+3, 16, 16, true);
    drawText(playing_ ? "||" : "|>", x+6, toolY+4, playCol, 11);
    ...
    // Zoom -/+
    fillRect(x+w-38, toolY+3, 16, 16, {50,50,50,255});
    drawBevel(x+w-38, toolY+3, 16, 16, true);
    drawText("-", x+w-34, toolY+4, colText, 11);
    fillRect(x+w-20, toolY+3, 16, 16, {50,50,50,255});
    drawBevel(x+w-20, toolY+3, 16, 16, true);
    drawText("+", x+w-16, toolY+4, colText, 11);
    ...
    // selected actor, or to the global (camera/screen) row if none is selected.
    {
        ...
        fillRect(addBtnX, addBtnY, 76, 16, {50,50,50,255});
        drawBevel(addBtnX, addBtnY, 76, 16, true);
        drawText("+ Event", addBtnX+4, addBtnY+2, {200,200,200,255}, 11);
    }
}
...
void CutsceneEditor::renderPropsPanel(int x, int y, int w, int h) {
    const SDL_Color bgPanel  = {30, 30, 35, 255};
    const SDL_Color colHead  = {180,200,255,255};
    const SDL_Color colLabel = {140,140,140,255};
    const SDL_Color colValue = {220,220,220,255};
    ...
    auto sep = [&](){
        drawLine(x+4, cy+2, x+w-4, cy+2, {60,60,60,255});
        cy += 6;
    };
    ...
    if (selectedEvent_ >= 0 && selectedEvent_ < (int)cs->events.size()) {
        ...
        // Delete button
        int delY = y + h - 20;
        fillRect(x+4, delY, 50, 16, {120,30,30,255});
        drawBevel(x+4, delY, 50, 16, true);
        drawText("Del Evt", x+7, delY+2, {220,220,220,255}, 11);
        return;
    }
    ...
    if (selectedActor_ >= 0 && selectedActor_ < (int)cs->actors.size()) {
        ...
        sep();
        int delY = y + h - 20;
        fillRect(x+4, delY, 60, 16, {120,30,30,255});
        drawBevel(x+4, delY, 60, 16, true);
        drawText("Del Actor", x+7, delY+2, {220,220,220,255}, 11);
        return;
    }
    ...
    int addDlgY = y + h - 40;
    fillRect(x+4, addDlgY, 80, 16, {40,60,40,255});
    drawBevel(x+4, addDlgY, 80, 16, true);
    drawText("+ Dialog", x+7, addDlgY+2, {200,200,200,255}, 11);
    if (selectedDialogSeq_ >= 0) {
        fillRect(x+4, addDlgY+20, 80, 16, {40,60,40,255});
        drawBevel(x+4, addDlgY+20, 80, 16, true);
        drawText("+ Line", x+7, addDlgY+22, {200,200,200,255}, 11);
    }
}
