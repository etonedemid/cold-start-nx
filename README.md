...
### v1.7.0 (2026-05-26)
- **Boss enrage** — bosses enrage at 50% HP: +30% speed, faster attack cadence; BossBrute gains a long-range charge (880px/s) when enraged; BossSniper switches to a 3-shot burst with spread; BossGunner fires a 12-round rapid-fire burst
- **Enemy speed** — all enemy base speeds increased ×1.5
- **Melee aggression overhaul** — melee enemies charge forward aggressively; orbit only during dash cooldown in a narrow band; body rotation clamped to ±30° from movement direction; each enemy spawns with a randomised orbit radius to prevent piling up
- **Contact damage** — enemies overlapping the player deal 1 HP of damage (respects the existing invuln window) and are physically pushed apart
- **Parry speed** — parried bullets now travel at sniper bullet speed
- **Milestone spawns** — guaranteed elite spawns at wave 40 (Brute), 60 (Sniper), and 80 (Gunner) with an orange announcement banner
- **Upgrade duplicates** — boolean upgrades are not re-offered until wave 25+, then freely duplicate
- **Dev console** — press `~` in singleplayer to open the terminal console; commands: `wave N`, `hp N`, `god`, `clear`, `spawn TYPE [N]`, `give NAME`, `bombs N`, `help`
...
- **Multiplayer splitscreen PvP hotfix** — networked subplayers now receive bullet damage and melee targeting as distinct local slots, with replicated HP/death updates so remote splitscreen opponents can actually be damaged
